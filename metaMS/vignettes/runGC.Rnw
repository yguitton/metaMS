...
\begin{itemize}
  ...
  \item rather than a feature-based analysis, as is the case with
    ...
    where the basic entity is a collection of (mz, I) pairs are
...
GCsettings[1:3]
@ 
The example shows that the settings used for this particular set of
samples are fine-tuned for a triple-quad GC. The \code{PeakPicking}
...
\begin{Schunk}
\begin{Sinput}
> result <- runGC(cdffiles, GCsettings, DB = DB)
\end{Sinput}
\end{Schunk}
...
<<>>=
xset.l <- peakDetection(cdffiles, settings = GCsettings$PeakPicking, 
                        convert2list = TRUE)
@ 
Since this part can take quite some time, it is operated in parallel
wherever possible, and shows some feedback to the user. The last
...
part of \pkg{metaMS} is a pseudospectrum, i.e. a set of peaks eluting
at the same time. This choice is motivated by several
...
two-second difference in retention time can be enough to see
separation. Secondly, fragmentation patterns for many compounds are
available in extensive libraries like the NIST
library~\cite{McLafferty2008}. In addition, the spectra are somewhat
...
\code{groupFWHM}:
<<>>=
allSamples <- lapply(xset.l, runCAMERA, chrom = GCsettings$chrom, 
                     settings = GCsettings$CAMERA)
@ 
Again, all the usual parameters for the \code{groupFWHM} function can
be included in the \code{settings\$CAMERA} argument. The most
...
results~\cite{Stein1994}. The first step in the comparison is based on
...
\code{matchIrrelevants} of the settings list) to be removed before
...
\code{betweenSamples} element of the settings list. Since the
...
retention time is larger.

\subsubsection{Output}

\subsection{Building a database of standards}
Standards may be injected as mixtures, provided that their retention
...
building blocks provided by \pkg{metaMS} in different ways. One of
them is discussed briefly as an example here, but several variations
are possible, depending on the characteristics of the experiment and
