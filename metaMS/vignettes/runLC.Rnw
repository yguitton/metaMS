...
list of settings which is passed as an argument to \code{runLC}. To
illustrate the structure of this list an example is included and
...
\item {\sc Peak picking}. The injections of the standards are
  ...
  specified in the \code{PeakPicking} element of the \code{settings}
