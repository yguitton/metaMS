## Matching grouped data with the database of standards
## Input: msp data object containing the samples, a msp data object
## containing the preprocessed DB, and some 
## settings in the form of a list.
## Output: a list of two elements, both nested lists:
## the first indicates which pseudospectrum in
## which sample is associated to which standard, the second contains
## the spectral match factors.

## New implementation, only doing spectral matching for those cases
## where the rt differences are below the threshold. Much faster!

matchSamples2DB <- function(xset.msp,
                            DB,
                            settings,
                            quick) {
  ...
  if (quick) { ## xset.msp has already been scaled
    ...
    ## standard compound. Slower, obviously.
    match.results <-
        lapply(1:length(xset.msp),
               function(ii) {
                 result <- matrix(0, length(DB), length(xset.msp[[ii]]))
