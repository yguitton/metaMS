...
match2ExtDB <- function(xsetList, extDB, settings)
{
  ...
  for (f.idx in 1:length(xsetList)) { ## loop over injections
    injection <- xsetList[[f.idx]]

    cnames <- injection$info[,1]
    
    for (c.idx in 1:length(cnames)) {  ## loop over standards
      ## if an extDB entry is found we check whether MW agrees with the
      ## monoMW given by Georg, and obtain the molecular formula.
      cas.idx <- which(sapply(extDB,
                              function(x, y)
                              x$CASNO == y,
      ...
      if (length(cas.idx) == 0) { 
        ...
        if (MW != round(injection$info[c.idx, "monoMW"])) {
          printWarning("Ext DB MW (", MW,
                       ") does not agree with MW from table (",
                       round(injection$info[c.idx, "monoMW"]),
                       ") for ", injection$info[c.idx, "Name"], ", CAS ",
                       injection$info[c.idx, "CAS"])
        }
      ...
      psp2 <- to.msp(injection$xset,
                     file = NULL,
                     settings = settings)
      psp2 <- lapply(psp2, function(x) x[order(x[,"mz"]),,drop = FALSE])

      ## remove features higher than the molecular mass plus 3 (to allow
      ## for isotopes)
      psp2 <- lapply(psp2, function(x) x[x[,"mz"] < MW + 4,,drop = FALSE])
      psp2 <- psp2[sapply(psp2, nrow) >= settings$minfeat]
      psp2 <- lapply(psp2,
                     function(x)
                     cbind(x[,1:2], rt = round(x[,3], 2)))
      
      rtrange <- injection$info$RT[c.idx] +
          c(-settings$rttol, settings$rttol)
      rts <- sapply(psp2, function(x) mean(x[,3]))
      idx <- which(rts < max(rtrange) & rts > min(rtrange))
      psp2 <- psp2[idx]

      ## now calculate matches with extDB - only retain those spectra
      ## having a match higher than the threshold
      if (length(idx) > 0) {
        if (length(cas.idx) > 0) {
          psp2 <- lapply(psp2, function(x) cbind(round(x[,1]), x[,-1]))
          match.results <- sapply(psp2, matchExpSpec, extDB[cas.idx],
                                  plotIt = FALSE, DB.treated = TRUE)
          
          if (is.matrix(match.results)) {
            best.match <- round(apply(match.results, 2, max), 4)
          } else {
            if (length(idx) > 1) { ## only one entry in ext DB!
              best.match <- round(match.results, 4)
            } else { ## only one pattern found in the experimental data
      ...
      if (length(psp2) > 0) {
        ## rt given is the one from Georg (in minutes); individual
        ## retention times are available in the third column of
        ## pspectrum, again in minutes
        ...
        ## Filtering this object is not necessary since it has aready
        ## been done at the psp2 level

        DBobj <- c(DBobj, msp.obj[best.match > settings$DBthreshold])
      } else {
        printWarning("No pattern found matching all criteria for",
                     injection$info[c.idx, "Name"],
                     "- CAS number", injection$info[c.idx, "CAS"])
      }
    }
  }
...
## conversion function, to get from a list of xset objects and
## corresponding meta-information to an msp object containing all
## relevant bits.
...
xset2msp <- function(xsetList, settings) {
  ...
  for (f.idx in 1:length(xsetList)) { ## loop over injections
    ...
    for (c.idx in 1:length(cnames)) {  ## loop over standards
      MW <- injection$info[c.idx, "monoMW"]
      psp2 <- to.msp(xsetList[[f.idx]]$xset,
                     file = NULL,
                     settings = settings)
      psp2 <- lapply(psp2, function(x) x[order(x[,"mz"]),,drop = FALSE])
      ## remove features higher than the molecular mass plus 3 (to allow
