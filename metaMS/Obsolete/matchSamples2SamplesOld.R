...
## Note that from July 30 2013 on, xset.msp contains SCALED spectra!

matchSamples2SamplesOld <- function(xset.work.orig,
                                 xset.msp,
                                 annotations = NULL,
                                 settingsList,
                                 plotIt = FALSE, ...) {
  if (!is.null(annotations)) {
    ## flag all unannotated pseudospectra
    noannot.idx <- mapply(function(x, y) which(sapply(y, length) == 0),
                          xset.msp,
                          annotations)
    
    xset.work <- mapply(function(x, y) x[y], xset.work.orig, noannot.idx)
  } else {
    xset.work <- xset.work.orig
    annotations <- lapply(xset.work,
                          function(x) vector(length(x), mode = "list"))
    noannot.idx <- lapply(xset.work, function(x) 1:length(x))
  }
  ...
  if (length(bigClusters) == 0) {
    ...
    for (cl in seq(along = clusters)) {
      ...
      central.idx <- which.max(Matrix:::rowSums(pattern.match.result[p.idx, p.idx]))
      pspc.DB[[ cl ]] <-              ## store this pseudospectrum!
        xset.msp[[ sample.idx[central.idx] ]][[ spec.idx[central.idx] ]]
      
      if (plotIt) {
        plotPseudoSpectrum(pspc.DB[[ cl ]],
      ...
      ## Stupid trick here: we use negative numbers for the annotation
      ...
      for (ii in seq(along = sample.idx)) {
        ...
        ## IS THIS A BUG?? old version (pre-Aug 2) of code...
...
match.unannot.patterns <- function(msp1, msp2, rt.width, simthresh) {
  rt1 <- sapply(msp1, function(x) mean(x[,"rt"]))
  rt2 <- sapply(msp2, function(x) mean(x[,"rt"]))

  rtdiffs <- abs(outer(rt1, rt2, "-"))
  close.idx <- which(rtdiffs < rt.width, arr.ind = TRUE)
  if (length(close.idx) > 0) {
    X1 <- msp1[close.idx[,1]]
    X2 <- msp2[close.idx[,2]]

    matchfactors <- mcmapply(mzmatch, X1, X2)
