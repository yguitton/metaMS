...
## if only one sample do not retcor ....
  ...
  ## The workflow is different depending on the type of retcor approach:
  ## for types linear and loess one needs the missing and extra and two runs of 
  ## grouping are performed (before and after retcor). for obiwarp only one grouping 
  ## is needed after the retcor
  ...
  if (settings$Retcor$method %in% c("loess", "linear")){
    ...
    xset <- do.call(group,c(list(object = xset,  bw = settings$bws[2], minsamp = myminsamp, minfrac = 0), 
      settings["mzwid"]))
    ## If Obiwarp ...... -------------------------------------------------------------------------- >
    ...
    xset <- do.call(retcor,retcorlist)
    ## Perform the second run of grouping
    xset <- do.call(group,c(list(object = xset,  bw = settings$bws[1], minsamp = myminsamp, minfrac = 0), 
      settings["mzwid"]))
