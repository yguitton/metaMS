...
getFeatureInfo <- function(stdDB, allMatches, sampleList) {
  ...
  if (length(pSpectra) > 0) {
    ...
    ## Next two instructions as suggested by Martin Morgan at http://stackoverflow.com/questions/4512465/what-is-the-most-efficient-way-to-cast-a-list-as-a-data-frame?rq=1
    f <- function(x) function(i) unlist(lapply(x, `[[`, i), use.names=FALSE)
    meta.info.df <- as.data.frame(Map(f(meta.info), fields),
                                  stringsAsFactors = FALSE)
  } else { ## no DB, or no hits from DB
    meta.info.df <- data.frame(Name = 1,
                               Class = 1,
                               rt.sd = 1,
                               rt = 1)[FALSE,]
  }

  ## Since the labels are negative, label -1 corresponds to the last
  ## unknown spectrum. This convention is also used in allAnnotations
  ## and in function constructExpPseudoSpectra
